...
import { 
  History, User, Calendar, Edit, Plus, Trash2, AlertCircle, Loader2
} from 'lucide-react';
...
export function EditHistoryDialog({ open, onOpenChange, scriptId, t }: EditHistoryDialogProps) {
  ...
  const fetchHistories = useCallback(async (page: number = 1) => {
    if (!scriptId) return;
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || t('editHistoryError') || '获取编辑历史失败');
      }
      ...
      setError(err instanceof Error ? err.message : (t('editHistoryErrorUnknown') || '获取编辑历史失败'));
    } finally {
      setLoading(false);
    }
  }, [scriptId, t]);
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <CardTitle className="text-sm">
                            <Badge 
                              variant="outline"
                              className={getOperationBadgeColor(history.operation)}
                            >
                          ...
                          <div className="flex items-center gap-4 text-xs text-gray-500 mt-1">
                            ...
                            <div className="flex items-center gap-1">
                              ...
                              <span>{formatDate(typeof history.operationTime === 'string' ? history.operationTime : new Date(history.operationTime).toISOString())}</span>
                            </div>
                          </div>
                        </div>
                      </div>
                  ...
                  {history.changes && history.changes.length > 0 && (
                    <CardContent className="pt-0">
                      <div className="space-y-2">
                        <h4 className="text-sm font-medium text-gray-700">{t('changesDetails')}：</h4>
                        {history.changes.map((change, changeIndex) => (
                          <div key={changeIndex} className="bg-gray-50 rounded-lg p-3">
                            <div className="flex items-center gap-2 mb-2">
                              <span className="text-sm font-medium text-gray-700">
                                {change.fieldDisplayNameCn || change.fieldDisplayName || getFieldDisplayName(change.field)}
                              </span>
                            </div>
                            <div className="grid grid-cols-2 gap-4 text-xs">
                              <div>
                                <span className="text-gray-500">{t('originalValue')}：</span>
                                <div className="mt-1 p-2 bg-red-50 border border-red-200 rounded text-red-800 font-mono">
                                  {formatValue(change.oldValue)}
                                </div>
                              </div>
                              <div>
                                <span className="text-gray-500">{t('newValue')}：</span>
