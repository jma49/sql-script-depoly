...
import { 
  History, User, Calendar, Edit, Plus, Trash2, AlertCircle, Loader2, Filter, RotateCcw, Search
} from 'lucide-react';
...
export function GlobalEditHistoryDialog({ open, onOpenChange, t }: GlobalEditHistoryDialogProps) {
  ...
  const fetchHistories = useCallback(async (params: {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : '获取编辑历史时发生未知错误';
          ...
          <CardContent className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              <div className="space-y-2">
                <Label htmlFor="script-name-filter" className="text-sm font-medium text-gray-700">
                  {t('scriptName')}
                </Label>
                <Input
              ...
              <div className="space-y-2">
                <Label htmlFor="operation-filter" className="text-sm font-medium text-gray-700">
                  {t('operationType')}
                </Label>
          ...
          <ScrollArea className="h-full pr-4">
            {loading ? (
              <div className="flex items-center justify-center py-16">
                ...
                <span className="text-lg text-gray-600">{t('loadingEditHistory')}</span>
              </div>
            ) : error ? (
              ...
              <div className="flex flex-col items-center justify-center py-16 text-gray-500">
                ...
                <span className="text-sm mt-2">尝试调整筛选条件或创建一些脚本操作</span>
              </div>
            ) : (
              <div className="space-y-4">
                {histories.map((history, index) => (
                  <Card key={history._id?.toString() || index} className="relative border border-gray-200 shadow-sm">
                    <CardHeader className="pb-4">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-3 mb-3 flex-wrap">
                              ...
                              <span className="font-semibold text-base text-gray-900 truncate">
                                {history.scriptSnapshot?.name || history.scriptSnapshot?.scriptId || 'Unknown Script'}
                              </span>
                            ...
                            <div className="flex items-center gap-6 text-sm text-gray-600">
                              <div className="flex items-center gap-2">
                                ...
                                <span>{history.userName || history.userEmail || history.scriptSnapshot?.author || 'Unknown'}</span>
                              </div>
                              <div className="flex items-center gap-2">
                                <Calendar className="w-4 h-4" />
                                <span>{formatDate(typeof history.operationTime === 'string' ? history.operationTime : history.operationTime.toISOString())}</span>
                              </div>
                    ...
                    {history.changes && history.changes.length > 0 && (
                      <CardContent className="pt-0">
                        <div className="space-y-3">
                          <h4 className="text-sm font-medium text-gray-800">{t('changesDetails')}：</h4>
                          <div className="space-y-3 max-h-48 overflow-y-auto">
                            {history.changes.map((change, changeIndex) => (
                              <div key={changeIndex} className="bg-gray-50 rounded-lg p-4 border border-gray-100">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                                  <div>
                                    <span className="text-gray-600 font-medium">{t('originalValue')}：</span>
                                    <div className="mt-2 p-3 bg-red-50 border border-red-200 rounded text-red-800 font-mono text-sm">
                                      {formatValue(change.oldValue)}
                                    </div>
                                  </div>
                                  <div>
                                    <span className="text-gray-600 font-medium">{t('newValue')}：</span>
