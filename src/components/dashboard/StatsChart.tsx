...
import { BarChart2, ArrowUpRight, Percent, Calendar, Info, CheckCircle, XCircle } from 'lucide-react';
...
export const StatsChart: React.FC<StatsChartProps> = ({
  ...
  const successRateValue = total > 0 ? Math.round((successCount / total) * 100) : 0;
  
  // 计算与昨天相比的增长率
  const growthRate = yesterdayChecks > 0 
        ...
        <div className="flex flex-col gap-4 flex-1">
          ...
          <div className="grid grid-cols-3 gap-2 text-center">
            ...
            <div className="bg-card/30 border border-border/30 rounded-md p-2">
              ...
              <p className={`font-semibold ${growthRate >= 0 ? 'text-green-500' : 'text-red-500'}`}>
          ...
          <div className="rounded-md border border-border/30 bg-card/30 p-4 flex-1 flex flex-col">
            ...
            <div className="flex justify-between items-end h-28 gap-4 mb-2 mt-2 flex-1">
              <div className="flex flex-col items-center justify-end flex-1">
                <div className="relative w-full rounded-t-md overflow-hidden flex justify-center items-end" style={{ minHeight: '80px' }}>
                  <div className="w-2/5 rounded-t-md relative" style={barSuccessStyle}>
                ...
                <div className="w-full pt-1 text-center border-t border-border/30 mt-1">
                  <span className="text-xs font-medium">{successRateValue}%</span>
                </div>
              </div>

              <div className="flex flex-col items-center justify-end flex-1">
                <div className="relative w-full rounded-t-md overflow-hidden flex justify-center items-end" style={{ minHeight: '80px' }}>
                  <div className="w-2/5 rounded-t-md relative" style={barFailureStyle}>
                    <span className="absolute -top-6 left-1/2 transform -translate-x-1/2 text-xs font-medium flex items-center">
                ...
                <div className="w-full pt-1 text-center border-t border-border/30 mt-1">
                  <span className="text-xs font-medium">{100 - successRateValue}%</span>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center justify-center gap-1 mt-2 text-xs text-muted-foreground">
              ...
              <span>{t('totalChecks')}: {successCount + failureCount}</span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="rounded-md border border-border/30 bg-card/30 p-3">
              <div className="flex justify-between">
                <div>
                  <p className="text-xs text-muted-foreground">{t('yesterday')}</p>
