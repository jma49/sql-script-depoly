...
export default function NewScriptPage() {
  ...
  const handleSaveScript = async () => {
    ...
    if (!formData.scriptId.trim() || !formData.name.trim() || !sqlContent.trim()) {
      toast.error(t('fillRequiredFieldsError') || 'Please fill all required fields: Script ID, Name, and SQL Content.');
      setIsSaving(false);
      return;
    }
    if (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(formData.scriptId)) {
      toast.error(t('invalidScriptIdError') || 'Invalid Script ID format. Use lowercase letters, numbers, and hyphens.');
      setIsSaving(false);
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.error(`${t('scriptSaveError') || 'Failed to save script:'} ${result.message || response.statusText}`);
      }
    } catch (error) {
      console.error("Error saving script:", error);
      toast.error(`${t('scriptSaveError') || 'Failed to save script.'} An unexpected error occurred.`);
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 md:p-6 lg:p-8 space-y-6">
      ...
      <div className="flex justify-end items-center space-x-3 pt-6 mt-6 border-t">
        <Button variant="outline" onClick={handleCancel} disabled={isSaving} className="w-28">
          {t('cancelButton') || 'Cancel'}
        </Button>
        <Button onClick={handleSaveScript} disabled={isSaving} className="w-28">
          {isSaving ? (t('savingStatusText') || 'Saving...') : (t('saveScriptButton') || 'Save Script')}
        </Button>
      </div>
    </div>
  );
