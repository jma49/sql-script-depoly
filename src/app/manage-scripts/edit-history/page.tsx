...
import { 
  History, User, Calendar, Edit, Plus, Trash2, AlertCircle, Loader2, Filter, RotateCcw, Search, ArrowLeft, FileText, Eye
} from 'lucide-react';
import { formatDate } from '@/components/dashboard/utils';
import { EditHistoryRecord, EditHistoryFilter } from '@/lib/edit-history-schema';
import { 
...
export default function GlobalEditHistoryPage() {
  ...
  const [operationFilter, setOperationFilter] = useState<EditHistoryFilter['operation']>('all');
  ...
  const fetchHistories = useCallback(async (params: {
    ...
    try {
      ...
      if (params.scriptName?.trim()) searchParams.set('scriptName', params.scriptName.trim());
      if (params.author?.trim()) searchParams.set('author', params.author.trim());
      if (params.operation && params.operation !== 'all') searchParams.set('operation', params.operation);
  ...
  }, [scriptNameFilter, authorFilter, operationFilter, dateFromFilter, dateToFilter, fetchHistories]);
  ...
  const getOperationText = (operation: string) => {
    switch (operation) {
      case 'create': return t('operationCreate');
      case 'update': return t('operationUpdate');
      case 'delete': return t('operationDelete');
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="space-y-3">
                  ...
                  <p className="text-lg text-muted-foreground leading-relaxed">
                    {t('editHistoryDescGlobal')} • {t('totalChanges')}: <span className="font-semibold text-primary">{totalRecords}</span>
                  </p>
                </div>
              </div>
            </div>
            ...
            <CardContent className="relative p-6 space-y-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div className="space-y-2">
                  <Label htmlFor="script-name-filter" className="text-sm font-semibold text-foreground/90 flex items-center gap-2">
                ...
                {/* Author Filter */}
                <div className="space-y-2">
                  <Label htmlFor="author-filter" className="text-sm font-semibold text-foreground/90 flex items-center gap-2">
                ...
                <div className="space-y-2">
                  <Label htmlFor="operation-filter" className="text-sm font-semibold text-foreground/90 flex items-center gap-2">
                    <div className="w-2 h-2 rounded-full bg-amber-400/60"></div>
                    {t('operationType')}
                  </Label>
                  <Select value={operationFilter} onValueChange={(value) => setOperationFilter(value as EditHistoryFilter['operation'])}>
                ...
                {/* Date From Filter */}
                <div className="space-y-2">
                  <Label htmlFor="date-from-filter" className="text-sm font-semibold text-foreground/90 flex items-center gap-2">
                ...
                {/* Date To Filter */}
                <div className="space-y-2">
                  <Label htmlFor="date-to-filter" className="text-sm font-semibold text-foreground/90 flex items-center gap-2">
          ...
          <Card className="group relative overflow-hidden border-2 border-border/20 bg-gradient-to-br from-card via-card to-card/90 shadow-lg hover:shadow-xl transition-all duration-500 hover:border-border/40">
            ...
            <CardHeader className="relative px-6 py-5 border-b border-border/30 bg-gradient-to-r from-muted/20 to-muted/10">
              <div className="flex items-center gap-4">
                ...
                <div className="space-y-2">
                  <CardTitle className="text-xl font-bold text-foreground leading-relaxed">
                    {histories.length > 0 ? `${histories.length} ${t('editHistoryRecords')}` : t('editHistoryTitle')}
                  </CardTitle>
                </div>
              </div>
            </CardHeader>
              ...
              {loading ? (
                <div className="flex items-center justify-center py-16">
                  <div className="text-center space-y-4">
                    ...
                    <p className="text-lg font-medium text-foreground">{t('loadingEditHistory')}</p>
                  </div>
                </div>
              ) : error ? (
                <div className="flex flex-col items-center justify-center py-16">
                  <div className="p-6 rounded-2xl bg-gradient-to-br from-red-50 to-red-100 dark:from-red-950/20 dark:to-red-900/20 border-2 border-red-200 dark:border-red-800 max-w-md mx-auto text-center">
                    ...
                    <p className="text-lg font-medium text-red-700 dark:text-red-400 mb-2">{t('errorTitle')}</p>
                    <p className="text-sm text-red-600 dark:text-red-500 mb-4">{error}</p>
                    <Button onClick={() => fetchHistories()} variant="outline" size="sm" className="shadow-md hover:shadow-lg transition-all duration-300">
                      ...
                      <TableBody className="divide-y divide-border/20">
                        {histories.map((history, index) => (
                          ...
                          >
                            ...
                            <TableCell className="px-4 py-5 font-medium max-w-56 leading-relaxed" title={history.scriptSnapshot?.name || history.scriptSnapshot?.scriptId}>
                            ...
                            <TableCell className="px-4 py-5 text-muted-foreground max-w-32 leading-relaxed" title={history.userName || history.userEmail}>
                              <div className="flex items-center gap-2">
                            ...
                            <TableCell className="px-4 py-5 text-muted-foreground max-w-40 font-mono text-sm leading-relaxed">
                              <div className="flex items-center gap-2">
                                ...
                                <span className="truncate">
                                  {formatDate(typeof history.operationTime === 'string' ? history.operationTime : history.operationTime.toISOString(), language)}
                                </span>
                              </div>
                            </TableCell>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="text-sm text-muted-foreground font-mono bg-muted/30 px-4 py-2 rounded-lg">
                      {t('pageNumber')} <span className="font-bold text-primary">{currentPage}</span> {t('of')} <span className="font-bold">{totalPages}</span>
                    </div>
                    <Button 
                      variant="outline" 
                      size="sm" 
        ...
        <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              {selectedHistory && (
                <span>
                  {getOperationText(selectedHistory.operation)} • {selectedHistory.scriptSnapshot?.name || selectedHistory.scriptSnapshot?.scriptId} • {formatDate(typeof selectedHistory.operationTime === 'string' ? selectedHistory.operationTime : selectedHistory.operationTime.toISOString(), language)}
                </span>
              )}
            </DialogDescription>
          </DialogHeader>
          ...
          {selectedHistory && (
            <div className="space-y-6 py-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 bg-muted/30 rounded-lg">
                <div>
                  <label className="text-sm font-medium text-muted-foreground">{t('operationType')}</label>
                  <div className="flex items-center gap-2 mt-1">
                    {getOperationIcon(selectedHistory.operation)}
                    <Badge variant="outline" className={getOperationBadgeColor(selectedHistory.operation)}>
                      {getOperationText(selectedHistory.operation)}
                    </Badge>
                  </div>
                </div>
                <div>
                  <label className="text-sm font-medium text-muted-foreground">{t('operationUser')}</label>
                  <div className="flex items-center gap-2 mt-1">
                    <User className="w-4 h-4" />
                    <span className="text-sm truncate" title={selectedHistory.userName || selectedHistory.userEmail || t('unknownUser')}>
                      {selectedHistory.userName || selectedHistory.userEmail || t('unknownUser')}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="p-4 bg-muted/30 rounded-lg">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    <p className="font-medium">{selectedHistory.scriptSnapshot?.name || t('unknown')}</p>
                  </div>
                  <div>
                    <label className="text-muted-foreground">{t('scriptNameCn')}</label>
                    <p className="font-medium">{selectedHistory.scriptSnapshot?.cnName || t('unknown')}</p>
                  </div>
                  <div>
                    <label className="text-muted-foreground">{t('fieldScriptId')}</label>
                    <p className="font-mono text-xs">{selectedHistory.scriptSnapshot?.scriptId || t('unknown')}</p>
                  </div>
                  <div>
                    <label className="text-muted-foreground">{t('author')}</label>
                    <p className="font-medium">{selectedHistory.scriptSnapshot?.author || t('unknown')}</p>
                  </div>
                </div>
              </div>
                    ...
                    {selectedHistory.changes.map((change, index) => (
                      <div key={index} className="border border-border/30 rounded-lg p-4 bg-background/50">
                        <div className="font-medium mb-3 text-sm">
                          {change.fieldDisplayNameCn || change.fieldDisplayName}
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          ...
                          <div>
                            <label className="text-xs text-muted-foreground font-medium">{t('newValue')}</label>
              ...
              {/* Description */}
              {(selectedHistory.descriptionCn || selectedHistory.description) && (
                <div className="p-4 bg-muted/30 rounded-lg">
